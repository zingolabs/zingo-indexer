...
mod wallet_basic {
    ...
    #[tokio::test]
    async fn receive_mining_reward() {
        for validator in ["zebrad", "zcashd"] {
            let mut test_manager = TestManager2::launch(validator, None, true, true)
                .await
                .unwrap();
    ...
    async fn send_to_orchard() {
        for validator in ["zcashd"] {
            ...
            assert_eq!(
                clients.recipient.do_balance().await.orchard_balance.unwrap(),
                250_000
            );
    ...
    async fn send_to_sapling() {
        for validator in ["zcashd"] {
            ...
            assert_eq!(
                clients.recipient.do_balance().await.sapling_balance.unwrap(),
                250_000
            );
    ...
    async fn send_to_transparent() {
        for validator in ["zcashd"] {
            ...
            assert_eq!(
                clients.recipient.do_balance().await.transparent_balance.unwrap(),
                250_000
            );
    ...
    async fn monitor_unverified_mempool() {
        for validator in ["zcashd"] {
            ...
            assert_eq!(
                recipient_client.do_balance().await.unverified_sapling_balance.unwrap(), 
                500_000
            );
            ...
            assert_eq!(
                recipient_client.do_balance().await.verified_sapling_balance.unwrap(), 
                500_000
            );
