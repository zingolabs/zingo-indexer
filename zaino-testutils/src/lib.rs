...
pub enum ValidatorConfig {
    /// Zcashd Config.
    ZcashdConfig(zingo_infra_testutils::validator::ZcashdConfig),
    /// Zebrad Config.
    ZebradConfig(zingo_infra_testutils::validator::ZebradConfig),
}
...
pub enum LocalNet {
    ...
    Zcashd(
        zingo_infra_testutils::LocalNet<
            zingo_infra_testutils::indexer::Empty,
            zingo_infra_testutils::validator::Zcashd,
        >,
    ),
    ...
    Zebrad(
        zingo_infra_testutils::LocalNet<
            zingo_infra_testutils::indexer::Empty,
            zingo_infra_testutils::validator::Zebrad,
        >,
    ),
}

impl zingo_infra_testutils::validator::Validator for LocalNet {
    ...
    fn launch(
        config: Self::Config,
    ) -> impl std::future::Future<Output = Result<Self, zingo_infra_testutils::error::LaunchError>> + Send
    {
        async move {
            match config {
                ValidatorConfig::ZcashdConfig(cfg) => {
                    let net = zingo_infra_testutils::LocalNet::<
                        zingo_infra_testutils::indexer::Empty,
                        zingo_infra_testutils::validator::Zcashd,
                    >::launch(
                        zingo_infra_testutils::indexer::EmptyConfig {}, cfg
                    )
                    .await;
    ...
    fn network(&self) -> zingo_infra_testutils::network::Network {
        match self {
    ...
    fn load_chain(
        ...
        validator_network: zingo_infra_testutils::network::Network,
    ) -> PathBuf {
        if chain_cache.to_string_lossy().contains("zcashd") {
            zingo_infra_testutils::validator::Zcashd::load_chain(
                chain_cache,
                validator_data_dir,
                validator_network,
            )
        } else if chain_cache.to_string_lossy().contains("zebrad") {
            zingo_infra_testutils::validator::Zebrad::load_chain(
                chain_cache,
                validator_data_dir,
                validator_network,
            )
...
pub struct TestManager {
    ...
    /// Network (chain) type:
    pub network: zingo_infra_testutils::network::Network,
...
impl TestManager {
    ...
    pub async fn launch(
        validator: &str,
        network: Option<zingo_infra_testutils::network::Network>,
        ...
        let network = network.unwrap_or(zingo_infra_testutils::network::Network::Regtest);
        ...
        let validator_config = match validator_kind {
            ValidatorKind::Zcashd => {
                let cfg = zingo_infra_testutils::validator::ZcashdConfig {
                    zcashd_bin: ZCASHD_BIN.clone(),
                    zcash_cli_bin: ZCASH_CLI_BIN.clone(),
                    rpc_port: Some(zebrad_rpc_listen_port),
                    activation_heights: zingo_infra_testutils::network::ActivationHeights::default(),
            ...
            ValidatorKind::Zebrad => {
                let cfg = zingo_infra_testutils::validator::ZebradConfig {
                    zebrad_bin: ZEBRAD_BIN.clone(),
                    network_listen_port: None,
                    rpc_listen_port: Some(zebrad_rpc_listen_port),
                    activation_heights: zingo_infra_testutils::network::ActivationHeights::default(),
                    miner_address: zingo_infra_testutils::validator::ZEBRAD_DEFAULT_MINER,
                    chain_cache,
                    network: zingo_infra_testutils::network::Network::Regtest,
                };
                ValidatorConfig::ZebradConfig(cfg)
            }
        };
...
mod tests {
    ...
    async fn launch_testmanager_zebrad_zaino() {
        let mut test_manager = TestManager::launch("zebrad", None, None, true, true, true, false)
            ...
            .unwrap();
        let mut grpc_client =
            zingo_infra_testutils::client::build_client(zingo_infra_testutils::network::localhost_uri(
                test_manager
                    .zaino_grpc_listen_port
    ...
    async fn launch_testmanager_zebrad_zaino_testnet() {
        let mut test_manager = TestManager::launch(
            "zebrad",
            Some(zingo_infra_testutils::network::Network::Testnet),
